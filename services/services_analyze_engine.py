...
def _debug_astro_result(label: str, astro_result: Any) -> None:
    try:
        data = astro_result if isinstance(astro_result, dict) else {}
        shichu = data.get("shichusuimei")
        shichu_keys = list(shichu.keys())[:20] if isinstance(shichu, dict) else []
        print("[analyze_engine][astro_result]", {
            "label": label,
            "top_keys": list(data.keys())[:30],
            "has_shichusuimei": isinstance(shichu, dict),
            "shichu_keys": shichu_keys,
...
def run_astro_calc(
    ...
    if astrology_system == "integrated_w_shichu":
        from services.shichusuimei_calc import calc_shichusuimei_from_payload
        return {
            "western": calc_western_from_payload(payload_a),
            "shichusuimei": calc_shichusuimei_from_payload(
...
def run_single(
    ...
    _debug_astro_result("run_single:after_calc", astro_result)
    astro_result = _attach_meta(astro_result, effective_meta)
    _debug_astro_result("run_single:after_attach_meta", astro_result)

    # transit データをAIプロンプトに渡せるよう埋め込む
    if isinstance(transit_data, dict):
        astro_result["transit"] = transit_data

    _debug_astro_result("run_single:before_generate_report", astro_result)
...
def build_handoff_logs(
    ...
    try:
        _debug_astro_result("build_handoff_logs:input", astro_result)
        if _build_structure_summary is not None:
            ...
            if isinstance(summary_obj, dict):
                print("[analyze_engine][structure_summary]", {
                    "top_keys": list(summary_obj.keys())[:30],
                    "has_shichusuimei": isinstance(summary_obj.get("shichusuimei"), dict),
                    "has_shichu_pillars": isinstance(summary_obj.get("pillars"), dict),
                })
    except Exception:
        summary_obj = None
