...
def _pillar_obj(value: Any) -> dict[str, str]:
    if isinstance(value, dict):
        return {
            "stem": _safe_text(value.get("stem") or value.get("kan") or value.get("ten_stem")),
            "branch": _safe_text(value.get("branch") or value.get("shi") or value.get("earthly_branch")),
        }
    if isinstance(value, (list, tuple)):
        return {
            "stem": _safe_text(value[0] if len(value) > 0 else ""),
            "branch": _safe_text(value[1] if len(value) > 1 else ""),
        }
    if isinstance(value, str):
        s = value.strip()
        return {"stem": s[:1], "branch": s[1:2] if len(s) > 1 else ""}
    return {"stem": "", "branch": ""}


def _hidden_stems_text(value: Any) -> str:
    items: list[str] = []
    if isinstance(value, list):
        for item in value:
            if isinstance(item, dict):
                stem = _safe_text(item.get("stem"))
                ten_god = _safe_text(item.get("ten_god"))
                if stem and ten_god:
                    items.append(f"{stem}（{ten_god}）")
                elif stem:
                    items.append(stem)
                else:
                    txt = _safe_text(item)
...
def extract_shichu_data(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if not pillars and isinstance(candidate.get("pillars"), dict):
        ...
        pillars = {key: _pillar_obj(raw_pillars.get(key)) for key in PILLAR_KEYS}
...
def render_shichu_table_html(data: dict[str, Any]) -> str:
    ...
    for key in PILLAR_KEYS:
        ...
        values.append({
            ...
            "hidden": _hidden_stems_text(hidden.get(key)) or "-",
